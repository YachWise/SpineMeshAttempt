...
class MeshAttachment: SKSpriteNode, Attachment
{
    let model: MeshAttachmentModel
    init(_ model: MeshAttachmentModel, _ texture: SKTexture)
    {
        self.model = model
        super.init(texture: texture, color: model.color.skColor, size: CGSize(width: model.height!, height: model.width!)
        )
        self.name = MeshAttachment.generateName(model.name)
        self.anchorPoint = CGPoint(x: 0.5, y: 0.5)
    }
